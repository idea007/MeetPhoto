 ...
 */
class TopicPhotosViewModel : BaseViewModel() {
    ...
    fun refreshTopicPhotos(slug: String) {
        curPageIndex = 1
        queryPhotos(slug, curPageIndex, PAGE_COUNT, refreshPhotosLiveData)
    }

    fun loadMoreTopicPhotos(slug: String) {
        queryPhotos(slug, curPageIndex, PAGE_COUNT, loadMorePhotosLiveData)
    }

    private fun queryPhotos(slug: String, pageIndex: Int, pageCount: Int, liveData: MutableLiveData<LiveDataPack<List<Photo>>>) {
        ...
        compositeDisposable.add(
            ...
            .getTopicPhotos(slug, pageIndex, pageCount).subscribeOn(Schedulers.io())
